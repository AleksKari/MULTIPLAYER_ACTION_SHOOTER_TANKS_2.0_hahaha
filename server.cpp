...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Multi-Lobby Server started on port " << port << "!" << std::endl;
    ...
    while (true) {
        ...
        for (size_t i = 0; i < clients.size(); ) {
            ...
            if (status == sf::Socket::Disconnected) {
                std::cout << "Client disconnected from lobby: " << clients[i]->lobby_code << std::endl;
                ...
                if (copy_packet >> packet_type) {
                    // Если это пакет регистрации лобби (например, тип 0 или 1 - Host/Join)
                    if (packet_type == 0 || packet_type == 1) { 
                        std::string code;
                        if (copy_packet >> code) {
                            clients[i]->lobby_code = code;
                            std::cout << "Client registered in Room/Lobby: " << code << std::endl;
                        }
                    }
                }
                
                // Пересылаем пакет только тем, кто находится в ТАКОЙ ЖЕ комнате (lobby_code)

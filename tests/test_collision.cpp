...
TEST(CollisionEntityProjectile, FarApartNoCollision) {
    EXPECT_FALSE(Collision::entity_projectile(Vec2(0, 0), 6, Vec2(1000, 1000), 32));
}
...
TEST(CollisionEntityProjectile, JustOutside) {
    // proj center=(3,3), entity center=(22,3) — расстояние=19, НЕ пересекаются (строго <)
...
TEST(CollisionEntityProjectile, DiagonalOverlap) {
    // proj center=(3,3), entity center=(3+9,3+9)=(12,12) — расстояние≈12.7 < 19 — пересекаются
    EXPECT_TRUE(Collision::entity_projectile(Vec2(0, 0), 6, Vec2(-4, -4), 32));
}

TEST(CollisionEntityProjectile, DiagonalNoCollision) {
    // proj center=(3,3), entity center=(17,17) — расстояние≈19.8 > 19 — НЕ пересекаются

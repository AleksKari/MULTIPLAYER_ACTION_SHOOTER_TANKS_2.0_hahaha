...
int main() {
  ...
  auto wep1 = std::make_unique<RicochetGun>();
  auto wep2 = std::make_unique<RicochetGun>();
  ...
  while (renderer.window().isOpen()) {
    ...
    if (currentState == GameState::Gaming) {
      ...
      clock.restart(); // чтобы dt не накапливалось, пока мы в меню
    }
